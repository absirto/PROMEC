...
import { PCPService } from '../../ServiceOrder/services/PCPService';
import { PurchaseService } from '../../ServiceOrder/services/PurchaseService';
  ...
  async create(req: Request, res: Response) {
    try {
      ...
      const order = await prisma.$transaction(async (tx) => {
        ...
        if (PCPService.hasValidPlanWindow(data.workCenter, data.plannedStartDate, data.plannedEndDate)) {
          ...
          if (conflicts.length > 0) {
             logger.warn(`Conflito de PCP detectado para OS ${traceCode}`);
          }
        }
      ...
      await NotificationService.notify({
        ...
        message: `A Ordem de Serviço ${traceCode} foi registrada com sucesso.`,
        type: 'SUCCESS',
        link: `/operations/service-orders/${order?.id}`
      });

      res.status(201).json(FinancialService.enrichFinancials(order));
    } catch (error: any) {
